 ...
 */
	...
	-- serializa por ch_nf sem bloquear: evita que 2 mensagens da mesma NF-e
	-- fiquem presas em lock (ex.: pacote reenviado/publicado 2x na fila de sync)
	-- hashtextextended (64 bits) reduz a chance de colisão vs hashtext (32 bits)
	_step_name := 'acquire_lock';
	IF NOT pg_try_advisory_xact_lock(hashtextextended(_ch_nf, 0)) THEN
		_result := jsonb_build_object(
			'success', false,
			'message', 'NF-e já está sendo processada em outra transação',
			'error_code', 'CONCURRENT_PROCESSING',
			'xml_id', _xml_id,
			'ch_nf', _ch_nf
		);
		RAISE WARNING '[%] Concorrência detectada para ch_nf % (XML ID %)', _step_name, _ch_nf, _xml_id;
		RETURN _result;
	END IF;
...
(
	...
	(
		...
		-- ICMS fantasma: XML da Reforma emitido só com IBS/CBS, sem grupo <ICMS>
		-- (ICMS obrigatório até 2033). Registra orig=0, cst='090', demais campos NULL.
		SELECT
			id_nfe,
			id_prod,
			0 AS orig,
		...
		WHERE
			orig IS NULL
			AND (xpath('/new:det/new:imposto/new:IBSCBS', r_i01_prod.det_node, _nfe_namespaces))[1] IS NOT NULL
	);
	/* FIM IMPOSTO ICMS */
	GET DIAGNOSTICS _records_processed = ROW_COUNT;
